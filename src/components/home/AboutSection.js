...
function AboutSection() {
          ...
          onEnter: () => {
            gsap.fromTo(counter, 
              ...
              { 
                ...
                snap: { innerText: 1 }, 
                onUpdate: function() {
                  counter.innerText = Math.ceil(this.targets()[0].innerText).toLocaleString('en-IN');
                }
              }
            );
          }
  ...
  return (
    <section id="about-section" ref={sectionRef} className="relative w-full bg-black text-white py-32 px-6 flex flex-col items-center z-20">
      
      <div className="text-center max-w-3xl mb-24">
        <h2 
          className="font-heading text-[#FC6840] text-[80px] md:text-[96px] mb-10 uppercase leading-none"
        ...
        <p className="font-body text-[#A3A3A3] text-lg md:text-xl leading-relaxed mb-6 font-medium">
          Alcheringa is the largest cultural fest in Northeast India, and one of the 
          biggest in the country — three days of music, art, dance, and madness that 
          brings together thousands of students from across India.
        </p>
        <p className="font-body text-[#A3A3A3] text-lg md:text-xl leading-relaxed font-medium">
          From electrifying concerts to nail-biting competitions, it's where talent meets 
          chaos meets unforgettable memories.
        </p>
      </div>
        ...
        {stats.map((stat, idx) => (
          ...
          >
            ...
            <div className="relative z-10 flex flex-col items-center text-black pt-2">
              ...
              {/* Size reduced here to text-3xl md:text-4xl while keeping font-bold */}
